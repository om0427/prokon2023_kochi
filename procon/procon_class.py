...
class Functions:
    ...
    def fileinit():
        ...
        with open("others.csv", mode="w", newline='') as file:
            ...
            file.writelines(str(Functions.pre_side)+"\n")
            for i in range(Functions.mason):
                file.writelines(str(i+1)+", 0, 0\n")
    ...
    def FieldAction():
        ...
        #for i in range(Functions.mason):
        #    if Functions.current_side==1:
        #        Functions.masons[i][0]=abs(Functions.masons[i][0])
        #    else:
        #        Functions.masons[i][0]=-abs(Functions.masons[i][0])
    ...
    def reload():
        ...
        with open("others.csv") as file:
            data=list(csv.reader(file))
        Functions.turn=int(data[0][0])
        Functions.myScore=int(data[0][1])
        Functions.enemyScore=int(data[0][2])
        Functions.current_side=int(data[1][0])
        ...
        for i in range(len(action_data)):
            action_list.append([])
            for k in range(len(action_data[i])):
                action_list[i].append(int(action_data[i][k]))
        Functions.actions=[]
        for i in range(Functions.mason):
            Functions.actions.append([0,0])
        for i in range(Functions.mason):
            for list1 in action_list:
                if list1[0]==i+1:
                    Functions.actions[i][0]=list1[1]
                    Functions.actions[i][1]=list1[2]
    ...
    def Save():
        ...
        with open("masons.csv", "w", newline='') as file:
            ...
            for i in range(Functions.height):
                writer.writerow(Functions.masons[i])
        
        with open("structures.csv", "w", newline='') as file:
            writer=csv.writer(file)
            for i in range(Functions.height):
                writer.writerow(Functions.structures[i])

        with open("territories.csv", "w", newline='') as file:
            writer=csv.writer(file)
            for i in range(Functions.height):
                writer.writerow(Functions.territories[i])

        with open("walls.csv", "w", newline='') as file:
            writer=csv.writer(file)
            for i in range(Functions.height):
                writer.writerow(Functions.walls[i])

        with open("others.csv", mode="w") as file:
            file.writelines(str(Functions.turn)+", "+str(Functions.myScore)+", "+str(Functions.enemyScore)+"\n")
            file.writelines(str(Functions.current_side)+"\n")
            for i in range(Functions.mason):
                file.writelines(str(i+1)+", 0, 0\n")
        os.chdir("../")

    #行動計画の読み込みとリセット、ターンインクリメント
    def Otherload():
        cwd=os.getcwd()
        os.chdir(cwd+"/current_status")
